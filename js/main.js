...
function renderShows() {
  ...
  for (let j = 0; j < favorites.length; j++) {
    const rememberedFav = shows.find(
      (show) => show.show.id === favorites[j].show.id
    );
    if (rememberedFav !== -1) {
      const rememberedFavPosition = shows.findIndex(
        (show) => show.show.id === favorites[j].show.id
      );
  ...
  const pEmptyResultsContent = document.createTextNode(
...
const pEmptyFavContent = document.createTextNode(
  'Your favorite-show list is empty ( ·﹏· )'
);
...
function getFromLocalStorage() {
  ...
  function renderFromLocalStorage() {
    ...
    function removeFromFav(event) {
      ...
      const favToRemove = favorites.findIndex(
        (favorite) => favorite.show.id === grandfatherId
      );
